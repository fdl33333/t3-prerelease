function dettEdit(winpos) {
    ...
    dettToolbar.addButton("tRef",2,"Aggiorna","reload.png","");
...
**/    
    ...
    dettToolbar.attachEvent("onClick", function(id) {
        ...
        switch (id) {
			...
			case "tRef" : 
			    dettGrid.clearAndLoad("../cn/dettGrid.php", function(){
					dettGrid.sortRows(7,"str","des");
			    });
				break;
    ...
    dettGrid.setHeader("Cognome,Nome,Saldo,Matricola,Tessera,Lingua,Tipo Crimine,Ultima Mod");
    // dettGrid.setColumnIds("serial,pinOrig,pin,dtCreate,notes");
    dettGrid.attachHeader("#text_filter,#text_filter,#numeric_filter,#text_filter,#text_filter,#select_filter,#select_filter,#text_filter");
    dettGrid.setColSorting("str,str,int,str,str,str,str,str");
	dettGrid.setColAlign("left,left,right,left,left,left,left,left");
    dettGrid.setInitWidths("120,120,80,100,60,100,150,*");
    dettGrid.setColTypes("ro,ro,ro,ro,ro,ro,ro,ro");
    ...
    anaToolbar.addButton("tRecup",5,"Lista Recuperi","Anchor.ico","Anchor.ico");   
...
//////////////////////////////////////////////////////////////////////////////////////// SAVING	    
    ...
    wlTb.addButton("tRecup",5,"Lista Recuperi","Anchor.ico","Anchor.ico");   
    wlTb.disableItem("tWL");
    wlTb.addSeparator("sep1",6);
	...
	function tbDo (id) {
		...
		switch(id) {
            ...
            case "tRecup" :
                dettLayout.cells("a").showView("recup");
                dettWin.setText("Ricuperi Manuali - " + dettName);
                recupGrid.clearAndLoad("../cn/recupGrid.php?dettId=" + dettGrid.getSelectedRowId(),  function(){
                    recupGrid.sortRows(0,"str","des");
                });
                break;
    ...
    histTb.addButton("tRecup",5,"Lista Recuperi","Anchor.ico","Anchor.ico");   
    histTb.disableItem("tHist");
    histTb.addSeparator("sep1",6);
    histTb.addButton("tRef",9,"Aggiorna","reload.png","");
    histTb.addSeparator("sep1",10);
    histTb.addButton("tRecupCreate",13,"Inserisci Recupero","left_arrow.png","left_arrow.png");
    ...
    histTb.attachEvent("onClick", function(id) {
        switch (id) {
			...
			case "tRecupCreate" :
				var callId = histGrid.getSelectedRowId();
				if (callId==null) {
					dhtmlx.alert("Selezionare la chiamata da recuperare");
					return;
				}
				if(histGrid.cells(callId, 3).getValue()=="R") {
					dhtmlx.alert("Selezionare chiamata Originale");
					return;
				}
				if(histGrid.cells(callId, 2).getValue()=="") {
					dhtmlx.alert("Selezionare una Chiamata Valida");
					return;
				}
				
				var ret = AGP(wsURL, {action:"RECUP_CREATE",callId: callId });							
				if (ret.status != 0) {
					dhtmlx.alert(ret.errMsg);
					return;
				} else  {
					dhtmlx.alert("Recupero inserito con successo");
				}
				break;
    ...
    recupTb = dettLayout.cells("a").attachToolbar();
    recupTb.setIconsPath("../assets/DHTMLX46/icons/");

    
    recupTb.addButton("tRet",1,"Lista Detenuti","User group.ico","User group-sel.ico");
    recupTb.addButton("tAna",2,"Anagrafica","Male.ico","Male-sel.ico");
    recupTb.addButton("tWL",3,"Numeri Consentiti","Phone.ico","Phone-sel.ico");   
    recupTb.addButton("tHist",4,"Storio Chiamate","List.ico","List-sel.ico");   
    recupTb.addButton("tRecup",5,"Lista Recuperi","Anchor.ico","Anchor.ico");   
    recupTb.addSeparator("sep1",6);
    recupTb.addButton("tDel",7,"Elimina Recupero","Delete.ico","Delete.ico");   

    
    recupTb.disableItem("tRecup");
	
    recupTb.attachEvent("onClick", function(id) {
        switch (id) {
        	case "tDel" :
        		var id = recupGrid.getSelectedRowId();
        		if (id==null) {
					dhtmlx.alert("Selezionare recupero da Eliminare");
					return;
        		}
        		
			    dhtmlx.confirm({
			        title:"Eliminazione Recupero Manuale"
			    ,    ok:"Si", cancel:"No"
			    ,    text:"Sicuro di voler eliminare questo recupero?"
			    ,    callback:function(result){
		                if (result) {
		                	var ret = AGP(wsURL,{action:"RECUP_DELETE", recupId:id});
		                	if (ret.status!=0) 
								dhtmlx.alert(ret.errMsg);
							else {
								dhtmlx.alert("Recupero Eliminato con successo");
				                recupGrid.clearAndLoad("../cn/recupGrid.php?dettId=" + dettGrid.getSelectedRowId(),  function(){
				                    recupGrid.sortRows(0,"str","des");
				                });
							}
	...
	recupGrid = dettLayout.cells("a").attachGrid();
	recupGrid.setImagePath("../assets/DHTMLX46/codebase/imgs/");   
	recupGrid.setHeader("Numero,Tipo,Descrizione,Creato");
	recupGrid.setInitWidths("90,150,200,*");
	recupGrid.setColTypes("ro,ro,ro,ro,ro");
    recupGrid.setColAlign("left,left,left,left");
    recupGrid.setColSorting("str,str,str,str");
    recupGrid.attachHeader("#text_filter,#select_filter,#text_filter,#text_filter");
	recupGrid.init();
